...
def main(args=None):
    '''
    print('Hi, rgbd node from read_img_tool.')
    color_dir = '/home/slam_py/o3d/livingroom1_color/'
    depth_dir = '/home/slam_py/o3d/livingroom1_depth_clean/'
    path_list=os.listdir(color_dir)
    path_list.sort(key=lambda x:int(x.split('.')[0]))
    for c_n in path_list:
        prefix = c_n.split('.')[0]
        d_n = '{}.png'.format(prefix)
        if not os.path.exists(depth_dir+d_n):
            continue
        print(c_n,d_n)
        img = cv2.imread(color_dir+c_n)
        depth = cv2.imread(depth_dir+d_n,cv2.IMREAD_ANYDEPTH)
        scale = np.max(depth)
        depth = depth/scale
        print(depth)
        cv2.imshow('img',img)
