...
def ga_optimize_3d_points(points3D, points2D_left, points2D_right, K_left, K_right, generations=50, pop_size=30, mutation_rate=0.1):
    ...
    【遗传算法原理说明】
    遗传算法（Genetic Algorithm, GA）是一种模拟自然界生物进化过程的全局优化算法。
    它通过“选择-交叉-变异”机制不断进化种群，使得个体（解）逐步逼近最优。
    在本函数中，每个个体是一组三维点的集合，适应度为所有点投影到左右相机像素平面后的总误差（误差越小适应度越高）。
    算法流程如下：
      1. 初始化：以原始三维点为基础，加入微小随机扰动，生成初始种群。
      2. 适应度评估：计算每个个体的投影误差总和，误差越小越优。
      3. 选择：选出适应度较高的一半个体作为父代。
      4. 交叉：随机配对父代，按随机切分点拼接生成新个体（子代）。
      5. 变异：对子代中的部分基因（点）加入微小随机扰动，增加多样性。
      6. 替换：新一代子代替换父代，进入下一轮迭代。
      7. 终止：达到最大迭代次数后，返回适应度最优的个体作为优化结果。
...
def pso_optimize_3d_points(points3D, points2D_left, points2D_right, K_left, K_right, 
    ...
    【粒子群算法原理说明】
    粒子群优化算法（Particle Swarm Optimization, PSO）是一种模拟鸟群觅食行为的全局优化算法。
    算法将每个解看作一个“粒子”，粒子在搜索空间中移动，其位置代表一个三维点集的解。
    每个粒子有速度和位置，粒子根据自身历史最优和全局最优动态调整运动方向，逐步逼近最优解。
    在本函数中，每个粒子是一组三维点的集合，适应度为所有点投影到左右相机像素平面后的总误差。
    算法流程如下：
      1. 初始化：以原始三维点为基础，加入微小随机扰动，生成初始粒子群，每个粒子有初始速度。
      2. 适应度评估：计算每个粒子的投影误差总和。
      3. 更新个体最优：若当前粒子优于历史最优，则更新其历史最优位置。
      4. 更新全局最优：若当前粒子优于全局最优，则更新全局最优位置。
      5. 速度与位置更新：每个粒子的速度由三部分组成——惯性、个体最优引力、全局最优引力，位置按速度更新。
      6. 迭代：重复上述过程，粒子群逐步收敛到最优解。
      7. 终止：达到最大迭代次数后，返回全局最优粒子的位置作为优化结果。
